  ...
  return (
    ...
    >
      <header className="p-6 border-b border-white/5 flex items-center gap-4">
        <button onClick={onBack} className="p-2 hover:bg-white/5 rounded-full transition-colors cursor-pointer">
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-hide">
        {requests.length > 0 && (
          <section className="space-y-4">
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">Friend Requests</h3>
            <div className="space-y-2">
              {requests.map((req) => (
                <div key={req.fromId} className="flex items-center justify-between p-4 bg-indigo-600/5 border border-indigo-500/20 rounded-2xl">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-2 bg-white/5 hover:bg-rose-500/20 text-zinc-400 hover:text-rose-400 rounded-xl transition-all cursor-pointer"
                    >
                      <X className="w-5 h-5" />
                    </button>
                  </div>
        ...
        <section className="space-y-4">
          ...
          {friends.length === 0 ? (
            <div className="text-center py-12 px-6 bg-zinc-900/50 rounded-3xl border border-white/5">
              <UserPlus className="w-12 h-12 text-zinc-700 mx-auto mb-4" />
              <p className="text-zinc-500 text-sm">No friends yet. Start a chat and add someone!</p>
            </div>
          ) : (
            <div className="space-y-2">
              {friends.map((friend) => (
                <div 
                  ...
                  className="flex items-center justify-between p-4 bg-zinc-900/50 border border-white/5 rounded-2xl hover:bg-zinc-900 transition-colors cursor-pointer group"
                >
                  <div className="flex items-center gap-3">
